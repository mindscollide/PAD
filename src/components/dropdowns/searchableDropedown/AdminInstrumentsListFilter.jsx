 ...
 */
export const AdminInstrumentsListFilter = ({
  setVisible,
  maininstrumentName,
  ...
  const { adminIntrumentListSearch, setAdminIntrumentListSearch } =
    useSearchBarContext();
  ...
  const handleSearchClick = () => {
    const { instrumentName, startDate, endDate } = localState;

    const searchPayload = {
      ...adminIntrumentListSearch, // Preserve other filters
    ...
    setAdminIntrumentListSearch(searchPayload);
    setLocalState(INITIAL_LOCAL_STATE);
    setClear(false);
    setVisible(false);
  };
  ...
  return (
    <>
      ...
      <Row gutter={[12, 12]}>
        ...
        <Col xs={24} sm={24} md={12} lg={12}>
          <DateRangePicker
            label="Date Range on Closing Period"
            size="medium"
