...
const AcordianTable = ({
  ...
  // 🔹 Because every column below sets an explicit `sortOrder`, AntD
  // treats sorting as fully controlled and will NOT run each column's
  // `sorter` function against dataSource itself — it only fires
  // onChange so the parent can update sortedInfo/icons. We have to sort
  // the rows ourselves here, using whichever column currently carries
  // an active sortOrder.
  const sortedData = useMemo(() => {
    if (!filteredData) return filteredData;

    const activeSortColumn = columns.find(
      (col) => col.sortOrder === "ascend" || col.sortOrder === "descend"
    );

    if (!activeSortColumn?.sorter) return filteredData;

    const sorted = [...filteredData].sort(activeSortColumn.sorter);
    return activeSortColumn.sortOrder === "descend" ? sorted.reverse() : sorted;
